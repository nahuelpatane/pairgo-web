...
const PROFILE_DATA = {
  ...
  skills: ["Kitchen Hand", "Barista", "Food Prep", "Bar Work", "Front of House", "Cleaning & Hygiene", "Stock Control", "Team Training"],
  certs: [
  ...
  workHistory: [
    { role: "Kitchen Hand", place: "Saltwater Café", city: "Byron Bay, NSW", period: "Jan – Apr 2026", rating: 5.0, managerName: "Tom S.", managerRole: "Head Chef", review: "Emma was outstanding. She picked up our kitchen procedures within 2 days and never needed reminding about hygiene standards. We would take her back any time.", emoji: "🌿", coverColor: "#ECFDF5", swapPartner: "Juan M. (Melbourne → Byron Bay)" },
    { role: "Barista", place: "The Beach Club", city: "Gold Coast, QLD", period: "Sep – Dec 2025", rating: 4.9, managerName: "Sarah K.", managerRole: "Floor Manager", review: "Very reliable and professional from day one. She integrated with the team immediately and our customers loved her energy and coffee skills.", emoji: "🏖️", coverColor: "#EFF6FF", swapPartner: "Mia T. (Cairns → Gold Coast)" },
    { role: "Barista", place: "Bondi Brew Co.", city: "Sydney, NSW", period: "Apr – Aug 2025", rating: 4.8, managerName: "Alex R.", managerRole: "Café Owner", review: "Excellent barista skills and a great attitude. Customers often came back specifically asking for her. Would recommend to any café owner.", emoji: "☕", coverColor: "#FFF8E1", swapPartner: "Lena W. (Cairns → Sydney)" },
  ],
};
...
export default function BackpackerProfile({ onBack }) {
  ...
  // Merge user signup data with the demo profile
  const profile = {
    ...PROFILE_DATA,
    name: user?.name || PROFILE_DATA.name,
    currentCity: user?.currentCity || PROFILE_DATA.currentCity,
    targetCity: user?.targetCity || PROFILE_DATA.targetCity,
    role: user?.currentRole || PROFILE_DATA.role,
  };

  const avgRating = (profile.workHistory.reduce((s, w) => s + w.rating, 0) / profile.workHistory.length).toFixed(1);
  ...
  return (
    <div style={{ background: "#F5F4F0", minHeight: "100vh", overflowX: "hidden" }}>
      <style>{`
        ...
        @media(max-width:768px){.content-grid{grid-template-columns:1fr;}.prof-header-inner{flex-direction:column!important;}.prof-actions{margin-top:16px;}}
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, padding: "16px 32px", display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(26,26,24,.88)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(255,255,255,.06)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
          <button onClick={onBack} style={{ display: "flex", alignItems: "center", gap: 6, color: "rgba(255,255,255,.55)", background: "none", border: "none", cursor: "pointer", fontFamily: "var(--font-b)", fontSize: 14, fontWeight: 500, transition: "color .2s", padding: 0 }}
      ...
      <div className="prof-content" style={{ maxWidth: 960, margin: "0 auto", padding: "0 24px" }}>
        ...
        {/* HEADER CARD */}
        <motion.div className="pcard" style={{ padding: "32px 36px", marginTop: -90, position: "relative", zIndex: 10, boxShadow: "0 8px 40px rgba(0,0,0,.1)" }}
          ...
          <div className="prof-header-inner" style={{ display: "flex", gap: 28, alignItems: "flex-start" }}>
            <div style={{ position: "relative", flexShrink: 0 }}>
              <img src={profile.photo} alt={profile.name} style={{ width: 104, height: 104, borderRadius: 24, objectFit: "cover", border: "4px solid #fff", boxShadow: "0 4px 20px rgba(0,0,0,.15)" }} />
              {profile.verified && <div style={{ position: "absolute", bottom: -6, right: -6, width: 28, height: 28, borderRadius: 8, background: "var(--forest)", display: "flex", alignItems: "center", justifyContent: "center", border: "2px solid #fff", fontSize: 12, color: "#fff", fontWeight: 700 }}>✓</div>}
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4, flexWrap: "wrap" }}>
                <h1 style={{ fontFamily: "var(--font-d)", fontSize: 28, fontWeight: 800, color: "var(--ink)", letterSpacing: "-1px" }}>{profile.name}</h1>
                <span style={{ fontSize: 22 }}>{PROFILE_DATA.flag}</span>
                <span style={{ padding: "3px 10px", borderRadius: 100, background: "var(--forest-soft)", color: "var(--forest)", fontFamily: "var(--font-b)", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1 }}>✓ Verified</span>
              </div>
              <div style={{ fontFamily: "var(--font-b)", fontSize: 15, color: "var(--ink-muted)", marginBottom: 20 }}>{profile.role} · {PROFILE_DATA.nationality}</div>
              <div style={{ display: "flex", gap: 32, flexWrap: "wrap" }}>
                {[{ label: "Rating", value: `★ ${profile.rating}`, color: "var(--gold)" }, { label: "Reviews", value: profile.reviewCount, color: "var(--ink)" }, { label: "Swaps done", value: profile.swapsCompleted, color: "var(--coral)" }, { label: "Years exp", value: `${profile.yearsExp}+`, color: "var(--ocean)" }].map((s, i) => (
                  <div key={i}>
            ...
            <div className="prof-actions" style={{ display: "flex", gap: 10, flexShrink: 0, alignItems: "flex-start" }}>
              <button style={{ display: "flex", alignItems: "center", gap: 7, padding: "11px 20px", borderRadius: 100, border: "1.5px solid rgba(0,0,0,.1)", background: "#fff", fontFamily: "var(--font-b)", fontSize: 14, fontWeight: 600, color: "var(--ink-soft)", cursor: "pointer", transition: "all .2s" }}
              ...
              <button style={{ display: "flex", alignItems: "center", gap: 7, padding: "11px 22px", borderRadius: 100, border: "none", background: "var(--coral)", color: "#fff", fontFamily: "var(--font-b)", fontSize: 14, fontWeight: 600, cursor: "pointer", transition: "all .2s" }}
        ...
        <div style={{ display: "flex", gap: 4, margin: "24px 0 20px", background: "#fff", padding: 4, borderRadius: 100, width: "fit-content", boxShadow: "0 2px 12px rgba(0,0,0,.05)" }}>
          {[{ id: "overview", label: "Overview" }, { id: "history", label: "Work History" }, { id: "reviews", label: "Reviews" }].map(t => (
        ...
        <AnimatePresence mode="wait">
          {tab === "overview" && (
            <motion.div key="overview" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.22 }}>
              <div className="content-grid">
                {/* Swap card */}
                <div style={{ background: "linear-gradient(145deg, #E8654A, #D4503A)", borderRadius: 24, padding: "32px 28px", color: "#fff", position: "relative", overflow: "hidden" }}>
                  <div style={{ position: "absolute", top: -30, right: -30, width: 140, height: 140, borderRadius: "50%", background: "rgba(255,255,255,.07)" }} />
                  <div style={{ position: "relative", zIndex: 1 }}>
                    <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: 2.5, opacity: .7, marginBottom: 20 }}>🎒 Looking to swap</div>
                    <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 24 }}>
                      <div>
                        ...
                        <div style={{ fontSize: 11, opacity: .6, marginTop: 2 }}>{profile.currentState || "QLD"}, Australia</div>
                      </div>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 11, opacity: .6, marginTop: 2 }}>{profile.targetState || "VIC"}, Australia</div>
                      </div>
                    </div>
                    {[{ icon: "📅", label: "Available from", value: profile.availableFrom }, { icon: "🛂", label: "Visa", value: profile.visaType }].map((item, i) => (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  ...
                  <div className="pcard" style={{ padding: "24px 32px" }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                      {[{ icon: "🌏", label: "Languages", value: profile.languages.join(" · ") }, { icon: "📍", label: "Current location", value: `${profile.currentCity}, ${profile.currentState || "QLD"}` }, { icon: "💼", label: "Current role", value: `${profile.role} @ ${profile.currentPlace}` }, { icon: "🛂", label: "Visa type", value: profile.visaType }].map((item, i) => (
                        <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                          <div style={{ width: 36, height: 36, borderRadius: 10, background: "var(--sand)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, flexShrink: 0 }}>{item.icon}</div>
                          <div>
              ...
              <div className="pcard" style={{ padding: "28px 32px", marginBottom: 48 }}>
                ...
                <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                  {profile.certs.map((c, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 20px", borderRadius: 14, background: c.color, fontFamily: "var(--font-b)", fontSize: 13, fontWeight: 600, color: "var(--ink-soft)" }}>
                      <span style={{ fontSize: 18 }}>{c.icon}</span><span>{c.name}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <motion.div key="history" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.22 }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 16, marginBottom: 48 }}>
                {profile.workHistory.map((job, i) => (
                  <motion.div key={i} className="pcard" style={{ overflow: "hidden" }} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}>
                    <div style={{ height: 5, background: "linear-gradient(90deg, var(--coral), var(--gold))" }} />
                    <div style={{ padding: "28px 32px" }}>
                      <div style={{ display: "flex", gap: 20, alignItems: "flex-start" }}>
                        <div style={{ width: 56, height: 56, borderRadius: 16, background: job.coverColor, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, flexShrink: 0 }}>{job.emoji}</div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontFamily: "var(--font-b)", fontSize: 14, color: "var(--ink-soft)", lineHeight: 1.7, background: "var(--sand)", borderRadius: 14, padding: "14px 18px", marginTop: 12, borderLeft: "3px solid var(--coral)" }}>
                            "{job.review}"
                            <div style={{ marginTop: 8, fontSize: 12, color: "var(--ink-muted)", fontStyle: "italic" }}>— {job.managerName}, {job.managerRole}</div>
                          </div>
                          <div style={{ display: "inline-flex", alignItems: "center", gap: 6, marginTop: 14, padding: "6px 14px", borderRadius: 100, background: "rgba(232,101,74,.08)", border: "1px solid rgba(232,101,74,.15)", fontFamily: "var(--font-b)", fontSize: 12, color: "var(--coral)" }}>
          ...
          {tab === "reviews" && (
            <motion.div key="reviews" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.22 }}>
                  ...
                  {[[5, 9], [4, 3], [3, 0]].map(([star, count]) => (
                    <div key={star} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                      ...
                      <div style={{ flex: 1, height: 8, borderRadius: 100, background: "rgba(0,0,0,.06)", overflow: "hidden" }}>
                        <div style={{ width: `${Math.round(count / profile.reviewCount * 100)}%`, height: "100%", borderRadius: 100, background: "var(--gold)", transition: "width .6s ease" }} />
                      </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 48 }}>
                {profile.workHistory.map((job, i) => (
                  <motion.div key={i} className="pcard" style={{ padding: "28px 32px" }} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 16 }}>
                      <div style={{ width: 48, height: 48, borderRadius: 14, background: job.coverColor, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0 }}>{job.emoji}</div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontFamily: "var(--font-b)", fontSize: 15, fontWeight: 600, color: "var(--ink)" }}>{job.managerName} · {job.managerRole}</div>
                        <div style={{ fontFamily: "var(--font-b)", fontSize: 12, color: "var(--ink-muted)" }}>{job.place}, {job.city} · {job.period}</div>
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: 4, flexShrink: 0 }}>
                        <span style={{ color: "var(--gold)", fontSize: 16 }}>★</span>
                        <span style={{ fontFamily: "var(--font-d)", fontSize: 22, fontWeight: 700, color: "var(--ink)" }}>{job.rating.toFixed(1)}</span>
                      </div>
                    </div>
                    <p style={{ fontFamily: "var(--font-b)", fontSize: 15, color: "var(--ink-soft)", lineHeight: 1.75 }}>"{job.review}"</p>
                  </motion.div>
                ))}
              </div>
            </motion.div>
